...
public class ProfileHandler extends Module {
    ...
    public void onEnable() {
        this.cache = CacheBuilder.newBuilder()
                ...
                .removalListener((RemovalListener<UUID, Profile>) n -> {
                    ...
                    if(n.getKey() != null && n.getValue() != null && n.getValue().getPlayer() != null && n.getValue().getPlayer().isConnected()) {
    ...
    public void setupPlayer(Profile profile) {
        ProxiedPlayer player = this.getPlugin().getProxy().getPlayer(profile.getUuid());
        if(player == null || !player.isConnected()) return;

        profile.getActiveGrants().forEach(grant -> grant.getRank().getFullPermissions().forEach(permission -> {
            if(permission.startsWith("-")) {
                player.setPermission(permission.replaceFirst("-", ""), false);
            } else {
