...
public final class PartyLang {
    ...
    static {
        INVITE_PREFIX.setColor(ChatColor.DARK_AQUA);
        ...
        ACCEPT_BUTTON.setColor(ChatColor.DARK_AQUA);
        ACCEPT_BUTTON.setHoverEvent(new HoverEvent(showText, acceptTooltip));

        INFO_BUTTON.setColor(ChatColor.DARK_AQUA);
    }
    ...
    public static HoverEvent hoverablePreviewTooltip(Party party) {
        ComponentBuilder builder = new ComponentBuilder("Members (").color(ChatColor.DARK_AQUA);
    ...
    private static List<String> getMemberPreviewNames(Party party) {
        ...
        for (int i = 0; i < Math.min(partySize, 6); i++) {
            ...
            displayNames.add(ChatColor.AQUA + UUIDUtils.name(member) + suffix);
        }
