...
final class MatchSpectatorLayoutProvider implements BiConsumer<Player, PotPvPLayoutProvider.TabLayout> {
        ...
        if (teams.size() == 2) {
            ...
            if (oldTeam != null) {
...

                {
                    ...
                    if (!duel) {
                        tabLayout.put(0, 3, ChatColor.LIGHT_PURPLE + ChatColor.BOLD.toString() + "Team One (" + teamOne.getAliveMembers().size() + "/" + teamOne.getAllMembers().size() + ")");
                    } else {
                        tabLayout.put(0, 3, ChatColor.LIGHT_PURPLE + ChatColor.BOLD.toString() + "Player One");
                    }
...

                {
                    ...
                    // we handle duels a bit differently
                    if (!duel) {
                        tabLayout.put(2, 3, ChatColor.AQUA + ChatColor.BOLD.toString() + "Team Two (" + teamTwo.getAliveMembers().size() + "/" + teamTwo.getAllMembers().size() + ")");
                    } else {
                        tabLayout.put(2, 3, ChatColor.AQUA + ChatColor.BOLD.toString() + "Player Two");
                    }
                ...
                for (MatchTeam team : match.getTeams()) {
                    for (UUID enemy : team.getAllMembers()) {
                        if (team.isAlive(enemy)) {
                            entries.put("&c" + UUIDUtils.name(enemy), PotPvPLayoutProvider.getPingOrDefault(enemy));
                        } else {
                            deadLines.put("&7&m" + UUIDUtils.name(enemy), PotPvPLayoutProvider.getPingOrDefault(enemy));
                        }
                    }
