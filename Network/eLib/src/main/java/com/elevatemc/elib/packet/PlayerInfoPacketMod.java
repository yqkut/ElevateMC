package com.elevatemc.elib.packet;
...
public class PlayerInfoPacketMod {
    ...
    public PlayerInfoPacketMod(String name,int ping,GameProfile profile,int action) {
        packet.setA(PacketPlayOutPlayerInfo.EnumPlayerInfoAction.values()[action]);
        packet.getB().add(new PacketPlayOutPlayerInfo.PlayerInfoData(profile, ping, WorldSettings.EnumGamemode.SURVIVAL, CraftChatMessage.fromString(name)[0]));
    }
