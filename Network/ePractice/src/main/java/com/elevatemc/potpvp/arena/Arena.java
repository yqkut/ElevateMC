...
import com.elevatemc.potpvp.util.AngleUtils;
 ...
 * See {@link com.elevatemc.potpvp.arena} for a comparision of
...
public final class Arena {
     ...
     * copied from.
     * @see com.elevatemc.potpvp.arena
     */
    ...
    public void takeSnapshot() {
        synchronized (chunkSnapshots) {
            forEachChunk(chunk -> chunkSnapshots.put(LongHash.toLong(chunk.getX(), chunk.getZ()), (ChunkSnapshot) chunk.takeSnapshot()));
        }
    }
    ...
    public void restore() {
        synchronized (chunkSnapshots) {
            World world = bounds.getWorld();
            chunkSnapshots.forEach((key, value) -> world.getChunkAt(LongHash.msw(key), LongHash.lsw(key)).restoreSnapshot((ChunkSnapshot) value));
