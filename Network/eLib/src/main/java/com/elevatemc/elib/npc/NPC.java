...
import com.elevatemc.elib.fake.impl.player.FakePlayerEntity;
import com.elevatemc.elib.skin.MojangSkin;
import com.elevatemc.elib.hologram.Hologram;
import com.elevatemc.elib.hologram.HologramLine;
...
public class NPC extends FakePlayerEntity {
    ...
    public void updateRideForViewers() {
        for (UUID uuid : this.getCurrentlyViewing()) {
            Player player = Bukkit.getPlayer(uuid);
            if (player == null) {
                continue;
    ...
    public void setMojangSkin(MojangSkin skin) {
        this.mojangSkin = skin;
        if (skin != null) {
            this.updateSkin(skin.getValue(), skin.getSignature());
        }
    }

    public NPCEntry toEntry() {
        List<String> hologramLines = this.hologram.getLines().stream().map(HologramLine::getLine).collect(Collectors.toList());
