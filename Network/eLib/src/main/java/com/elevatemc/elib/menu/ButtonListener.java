    ...
    public void onButtonPress(InventoryClickEvent event) {
        ...
        if (openMenu != null) {
            ...
            if (openMenu.getButtons().containsKey(event.getSlot())) {
                ...
                player.setMetadata("CLICKED_BUTTON",new FixedMetadataValue(eLib.getInstance(),player.getUniqueId().toString()));
                ...
                if (event.isCancelled()) {
                    eLib.getInstance().getServer().getScheduler().runTaskLater(eLib.getInstance(), player::updateInventory, 1L);
                }

                player.removeMetadata("CLICKED_BUTTON", eLib.getInstance());
