...
import com.elevatemc.potpvp.match.listener.MatchKitSelectionListener;
...
public final class Match {
    ...
    // https://github.com/ElevateOrb/PotPvP-SI/issues/19
    ...
    // it'll only be populated if the GameMode allows building in the first place.
    ...
    @Getter
    private Map<UUID, Integer> totalTags = Maps.newHashMap();
    @Getter
    ...
    public Match(com.elevatemc.potpvp.gamemode.GameMode gameMode, Arena arena, List<MatchTeam> teams, boolean ranked, boolean allowRematches) {
    ...
    private void saveState() {
        if (gameMode.getBuildingAllowed())
    ...
    void startCountdown() {
        ...
        for (MatchTeam team : this.getTeams()) {
            ...
            for (UUID playerUuid : team.getAllMembers()) {
                ...
                Location spawn = (teams.size() > 2 ? arena.getTeam1Spawn() : (team == teams.get(0) ? arena.getTeam1Spawn() : arena.getTeam2Spawn())).clone();
                Vector oldDirection = spawn.getDirection();

                if (handedTo == team && this.gameMode.equals(GameModes.TRAPPING)) {
                    spawn = arena.getTeam1Spawn();
                    player.setMetadata("TRAPPER", new FixedMetadataValue(PotPvPSI.getInstance(), true));
                } else if (handedTo != team && this.gameMode.equals(GameModes.TRAPPING)) {
                    player.removeMetadata("TRAPPER", PotPvPSI.getInstance());
                    spawn = arena.getTeam2Spawn();
                }
                ...
                PatchedPlayerUtils.resetInventory(player, org.bukkit.GameMode.SURVIVAL);
        ...
        new BukkitRunnable() {
            ...
            int countdownTimeRemaining = gameMode.equals(GameModes.SUMO) ? 3 : 5;
            ...
            public void run() {
                ...
                messageAll("Match starting in " + ChatColor.DARK_AQUA + countdownTimeRemaining + ChatColor.WHITE + "...");
    ...
    public void endMatch(MatchEndReason reason) {
        ...
        try {
            for (MatchTeam matchTeam : this.getTeams()) {
                matchTeam.removeRally();
                for (UUID playerUuid : matchTeam.getAliveMembers()) {
                    MatchKitSelectionListener.appliedKits.remove(playerUuid);
                }
                for (UUID playerUuid : matchTeam.getAllMembers()) {
                    ...
                    if (this.gameMode.equals(GameModes.TRAPPING)) {
                        for (Ability value : PotPvPSI.getInstance().getAbilityHandler().getAbilities().values()) {
                            value.removeCooldown(player);
                        }
                    }

                    postMatchPlayers.computeIfAbsent(playerUuid, v -> new PostMatchPlayer(player, gameMode.getHealingMethod(), totalHits.getOrDefault(player.getUniqueId(), 0), longestCombo.getOrDefault(player.getUniqueId(), 0), totalTags.getOrDefault(player.getUniqueId(), 0), missedPots.getOrDefault(player.getUniqueId(), 0)));
                }
            }

            messageAll(ChatColor.DARK_GREEN.toString() + ChatColor.BOLD + "✔ " + ChatColor.GREEN + "Match ended.");
    ...
    private void terminateMatch() {
        ...
        if (gameMode.getBuildingAllowed()) {
            Bukkit.getLogger().info("Restored arena for match " + this.get_id());
            arena.restore();
        }
    ...
    public void addSpectator(Player player, Player target, boolean fromMatch) {
        if (!fromMatch && state == MatchState.ENDING) {
            player.sendMessage(ChatColor.DARK_RED + "✖ " + ChatColor.RED + "This match is no longer available for spectating.");
            return;
        }
        ...
        if (!fromMatch) {
            ...
            player.sendMessage(ChatColor.YELLOW + "Now spectating " + ChatColor.GOLD + getSimpleDescription(true) + ChatColor.YELLOW + "...");
        ...
        PatchedPlayerUtils.resetInventory(player, org.bukkit.GameMode.CREATIVE, true); // because we're about to reset their inv on a timer
...
//        }
    ...
    public void markDead(Player player) {
        ...
        if (gameMode.equals(GameModes.PEARL_FIGHT)) {
            MatchKitSelectionListener.appliedKits.remove(player.getUniqueId());
        }
        ...
        postMatchPlayers.put(player.getUniqueId(), new PostMatchPlayer(player, gameMode.getHealingMethod(), totalHits.getOrDefault(player.getUniqueId(), 0), longestCombo.getOrDefault(player.getUniqueId(), 0), totalTags.getOrDefault(player.getUniqueId(), 0), missedPots.getOrDefault(player.getUniqueId(), 0)));
        checkEnded();
    }
    ...
    public String getSimpleDescription(boolean includeRankedUnranked) {
        ...
        if (includeRankedUnranked) {
            ...
            return players + " (" + rankedStr + " " + gameMode.getName() + ")";
        } else {
            return players;
        }
    }
    ...
    /**
     * Checks if a block can be broken in this match. Only used if the GameMode
     * allows building.
     */
    public boolean canBeBroken(Block block, Player player) {
        return (gameMode.equals(com.elevatemc.potpvp.gamemode.GameModes.TRAPPING) && player.hasMetadata("TRAPPER")) || gameMode.getId().equals("SPLEEF") && (block.getType() == Material.SNOW_BLOCK || block.getType() == Material.GRASS || block.getType() == Material.DIRT) || placedBlocks.contains(block.getLocation().toVector().toBlockVector());
    }

    public boolean canBeBroken(Block block) {
        return (gameMode.equals(com.elevatemc.potpvp.gamemode.GameModes.TRAPPING)) || gameMode.getId().equals("SPLEEF") && (block.getType() == Material.SNOW_BLOCK || block.getType() == Material.GRASS || block.getType() == Material.DIRT) || placedBlocks.contains(block.getLocation().toVector().toBlockVector());
    }

    public List<Player> findAllPlayers() {
        // Fuck this shit so aids whatever bro gotta do it this way

        final List<Player> toReturn = new ArrayList<>();

        for (MatchTeam team : this.getTeams()) {
            for (UUID aliveMember : team.getAllMembers()) {
                final Player player = PotPvPSI.getInstance().getServer().getPlayer(aliveMember);
        ...
        for (UUID spectator : this.getSpectators()) {
            final Player player = PotPvPSI.getInstance().getServer().getPlayer(spectator);
