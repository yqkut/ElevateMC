...
public final class PartyInviteCommand {
    ...
    public static void partyInvite(Player sender, @Parameter(name = "player") Player target) {
        ...
        if (sender == target) {
            sender.sendMessage(ChatColor.DARK_RED + "✖ " + ChatColor.RED + "You cannot invite yourself to your own party.");
            return;
        }

        if (sender.hasMetadata("modmode")) {
            sender.sendMessage(ChatColor.DARK_RED + "✖ " + ChatColor.RED + "You cannot do this while in mod mode!");
            return;
        }

        if (party != null) {
            if (party.isMember(target.getUniqueId())) {
                sender.sendMessage(ChatColor.DARK_RED + "✖ " + ChatColor.RED + target.getName() + " is already in your party.");
                return;
            }

            if (party.getInvite(target.getUniqueId()) != null) {
                sender.sendMessage(ChatColor.DARK_RED + "✖ " + ChatColor.RED + target.getName() + " already has a pending party invite.");
                return;
            }
        }

        if (partyHandler.hasParty(target)) {
            sender.sendMessage(ChatColor.DARK_RED + "✖ " + ChatColor.RED + target.getName() + " is already in another party.");
            return;
        }
        ...
        if (party.getMembers().size() >= Party.MAX_SIZE && !sender.isOp()) { // I got the permission from "/party invite **" below
            sender.sendMessage(ChatColor.DARK_RED + "✖ " + ChatColor.RED + "Your party has reached the " + Party.MAX_SIZE + " player limit.");
            return;
        }
