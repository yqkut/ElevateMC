...
public class CommandNode {
    ...
    public FancyMessage getUsage() {
        ...
        if (!flags.isEmpty()) {
            usage.then("(").color(ChatColor.DARK_AQUA);
            ...
            for (FlagData data : flags) {
                ...
                usage.then("-" + name).color(ChatColor.DARK_AQUA);
        ...
        if (!parameters.isEmpty()) {
            ...
            for(int index = 0; index < parameters.size(); ++index) {
                ...
                usage.then((required ? "<" : "[") + data.getName() + (data.isWildcard() ? "..." : "") + (required ? ">" : "]") + (index != parameters.size() - 1 ? " " : "")).color(ChatColor.DARK_AQUA);
            }
        }
        ...
        if (this.method == null) {
            ...
            for (Data unknownData : this.parameters) {
                ...
                if (unknownData instanceof FlagData) {
                    ...
                    ParameterType<?> type = eLib.getInstance().getCommandHandler().getParameterType(parameterData.getType());
            ...
            try {
                ...
                int executionThreshold = eLib.getInstance().getConfig().getInt("Command.TimeThreshold", 10);

                if (!this.async && this.logToConsole && stopwatch.elapsed(TimeUnit.MILLISECONDS) >= (long)executionThreshold) {
                    eLib.getInstance().getLogger().warning("Command '/" + this.getFullLabel() + "' took " +
    ...
    public List<String> getSubCommands(CommandSender sender, boolean print) {
        ...
        if (!commands.isEmpty() && print) {
            ...
            for (String command : commands.stream().sorted(String::compareTo).collect(Collectors.toList())) {
                sender.sendMessage(ChatColor.DARK_AQUA + command);
            }
